...
public class TestBase {
    ...
    @Step("Открываю сайт")
    public void openURL() {
        open("https://mokkano.ru/");
    }

    @Step("Выбираю рандомный город. Выбор городов реализован через Drop Down")

    public void restSelect() {
        SelenideElement modalContent = $x("//div[contains(@class, \"modal-arora-content\")]");
        modalContent.shouldBe(visible);
        List<SelenideElement> terminalSwitch = elements(By.xpath("//li[@class = \"city-select-item\"]/a[contains(text(), \"\") and not (text() = \"Ногинск\")]"));
    ...
    public void mathRandomHead() {
        $x("//div[@class = \"cat-list\"]").shouldBe(visible);
        List<SelenideElement> mathRandomHead = elements(By.xpath("//div[@class = \"cat-list\"]/ul/li/a[contains(text(), \"\") and not (contains(text(), \"Конструктор WOK\")) and not (@href = \"/actions\") and not (contains(text(), \"Пицца наборы\"))]"));
    ...
    public void pickRandCards() {
        List<SelenideElement> clickRandomCards = elements(By.xpath("//div[contains(@class, \"productBox\") and not (contains(@class, \"action-wrapper\")) and not (contains(@class, \"in-stop-list\"))]//a[contains(@class, \"add-to-basket\")]"));
    ...
    public void selectDeliveryTypePickUp() {
        SelenideElement typePickUp = $(By.xpath("//div[contains(@class, \"options-box\")]//span[contains(text(), \"Самовывоз\")]"));
    ...
    public void goNextOrderReg(){
        $x("//div[contains(@class, \"item-cart-buttons\")]/button[@class = \"bt_link\"]").click();
    }
